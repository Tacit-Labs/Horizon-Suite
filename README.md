...
- **Auto-Focus Closest Quest** — Optionally super-track whichever tracked quest is physically closest (works with any sort mode). Optional **Include Untracked Quests** lets a closer log quest take focus even if it isn't on the tracker. Toggle on the fly with `/h focus autofocus` or a keybinding under Key Bindings → Horizon Suite.
