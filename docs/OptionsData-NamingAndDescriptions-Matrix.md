...
| Global Toggles | Global UI scale              | Scale all UI elements (50–200%).                                            | Doesn't change your configured values, only the effective display scale.            |
| Global Toggles | Per-module scaling           | Separate scale slider per module.                                           | Overrides the global scale with individual sliders for Focus, Presence, Vista, etc. |
