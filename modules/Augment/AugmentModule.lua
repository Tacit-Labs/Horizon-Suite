...
addon:RegisterModule("augment", {
    title       = "Augment",
    description = "Loot notifications, vendor automation, self-highlight, Talking Head customisation, achievement tracking, and flight-time estimates.",
    order       = 30,
    ...
    OnEnable = function()
        if addon.Augment then
            ...
            local flightTimerOn = not GetDB or GetDB("flightTimerEnabled", false) ~= false
            ...
            if flightTimerOn and addon.Augment.FlightTimer then addon.Augment.FlightTimer.Enable() end
        end
    end,
    ...
    OnDisable = function()
        if addon.Augment then
            ...
            if addon.Augment.FlightTimer then addon.Augment.FlightTimer.Disable() end
