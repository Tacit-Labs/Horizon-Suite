--[[
    Horizon Suite - Augment / FlightTimer - Bundled Flight-Time Dataset
    Per-node taxi flight durations (seconds), by faction, used to show an
    estimated time-of-arrival on taxi/flight-map tooltips before a route is
    taken, and as the multi-hop pathfinding input in
    AugmentFlightTimerEstimator.lua for routes with no direct entry below.

    Sourced from InFlight (https://www.curseforge.com/wow/addons/inflight),
    MIT licensed by TotalPackage, Deranjata, and LudiusMaximus. The full
    license text is reproduced in THIRD_PARTY_LICENSE.txt in this folder, per
    the MIT license's attribution requirement. Retail-only: InFlight's
    Classic-era node-name dataset and legacy saved-variable converter were
    not ported, as Horizon Suite does not support Classic.

    This table is read-only at runtime. Corrections learned from actual play
    are stored separately (see AugmentFlightTimerState.lua) and overlaid on
    top of this data at lookup time, never written back into it.
]]
