...
Plan 13 made Blizzard's docked input line look like Echo's own reply box, and added a Collapse setting that folds the tile column into the Echo icon. It also made every tile image fill its rounded square.

**Placement.** With the card shown and the docked line visible, the line sits over the card's reply-box slot (`Card.ReplySlot()`), from the reply box's left edge to the send button's right edge. The card hides its own reply box, mode chip and send button whenever the line covers it, whichever chat the line points at, and its message area keeps its normal bottom. With no card shown, the line stays beside the Echo icon and takes the same look. When the line deactivates and always-visible is off, the card's own box comes back.

**Look.** Behind the line, Echo draws the reply box's fill in its radius (`Echo.Round.SMALL`), with no chat-coloured border. The chat's colour moves to a rounded chip behind Blizzard's header ("Say:", "Guild:", "Whisper Brisa:"), coloured from `ChatTypeInfo` at alpha 0.22 and 22px high. The chip reaches 8px before the header and only 2px after it, because Blizzard starts the typed text right after the header. A header whose width is secret or unreadable gets a 48px chip. The header, and `headerSuffix` where it exists, takes the card's chip label colour and Echo's font at size 10. A "Reply…" hint sits after the chip while the box is empty and has no IME text; it updates from a `HookScript("OnTextChanged")` post-hook, which this plan added to plan 12's allowed list. The chip updates in the `UpdateHeader` post-hook. Every change is recorded and restored when docking goes off.
...
**The icon's hover.** Hovering the Echo icon opens the stack only when collapse is `"off"`. While collapsing, the same hover unfolds the column instead, and the tiles keep their own stack peek.
