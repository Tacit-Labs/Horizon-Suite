...
The ***only*** place that this will be different is in any external hooks, WoW API format-matching, or folders prefaced with a "`.`", 
Everywhere else, mention if you feel the pattern is not followed appropriately.
...
**General Example**:
This is meant as a rough guideline, not as an ultimate authority.
...
|Example|Reasoning|
|-|-|
|<span style="color:#0BD151;">PlayerClass</span> `= GetPlayerClass()`|
|<span style="color:#EE4B2B;">PlayerCurrentClass</span> `= GetPlayerClass()`|
|<span style="color:#EE4B2B;">CurrentClass</span> `= GetPlayerClass()`|
|<span style="color:#EE4B2B;">CharacterClass</span> `= GetPlayerClass()`|
|<span style="color:#EE4B2B;">Class</span> `= GetPlayerClass()`|
