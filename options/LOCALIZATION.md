...
Compare LocaleBase.lua with each locale file to see which need the new key. Open each locale file (esES, deDE, frFR, koKR, etc.) and add the new key in the same section as LocaleBase. Use the key as the value if untranslated (it will fall back to English at runtime).
